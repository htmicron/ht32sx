...
#define WAKE_UP_TIME			30 							/**< Wkp time in SECONDS */

#define MAX_N_DEVICES			3							/**< Max numbers of devices in the P2P network. The last one will be the gateway. */
#define FIRST_ADDRESS			(uint8_t)0x01				/**< First device address. */

#define GATEWAY_ID 				(uint8_t)MAX_N_DEVICES		/**< Gateway device P2P ID. */

#define SFX_CREDENTIALS_SIZE	48							/**< Sigfox credentials buffer size. */
#define SFX_ENVIRONMENT_SIZE	4							/**< Sigfox environment buffer size. */
#define SFX_PAYLOAD_SIZE		12							/**< Sigfox payload buffer size. */
#define SFX_CONTEXT_SIZE		4							/**< Sigfox context buffer size. */
 ...
 */
typedef struct {
	uint8_t sfx_credentials[SFX_CREDENTIALS_SIZE];			/**< Sigfox credentials buffer */
	uint8_t sfx_environment[SFX_ENVIRONMENT_SIZE];			/**< Sigfox environment buffer */
	uint8_t payload_type;									/**< P2P payload type */
	uint8_t data[SFX_PAYLOAD_SIZE];							/**< Sigfox data */
	uint8_t sfx_context[SFX_CONTEXT_SIZE];					/**< Sigfox context buffer */
} __attribute__((packed)) HT_DR_Payload;
